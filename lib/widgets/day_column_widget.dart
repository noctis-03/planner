...
class _DayColumnWidgetState extends State<DayColumnWidget> {
  ...
  void didChangeDependencies() {
    ...
    // 최초 1회 스크롤 컨트롤러를 provider 에 등록
    if (!_registered) {
      _registered = true;
      final provider = context.read<PlannerProvider>();
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          provider.registerDayScrollController(widget.day.id, _vCtrl);
        }
      });
    }
  }
  ...
  void dispose() {
    try {
      final provider = context.read<PlannerProvider>();
      provider.unregisterDayScrollController(widget.day.id);
      provider.unregisterMatchedKey(widget.day.id);
    } catch (_) {}
    _vCtrl.dispose();
    super.dispose();
  }
  ...
  void _syncMatchedKey(PlannerProvider provider) {
    final id = _matchedIdOf(provider);
    if (id == null) {
      provider.unregisterMatchedKey(widget.day.id);
    } else {
      provider.registerMatchedKey(widget.day.id, _keyFor(id));
    }
  }
  ...
  void _maybeAlign(PlannerProvider provider) {
    ...
    if (provider.alignOriginDayId == widget.day.id) return;
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) _syncMatchedKey(provider);
    });
