...
class PlannerProvider extends ChangeNotifier {
  ...
  /// 각 요일 컬럼의 매칭 블록의 화면 글로벌 좌표(중심).
  /// (현재 곡선 연결에는 blockLefts/blockRights를 사용하며, 이 필드는 예비용)
  final Map<String, Offset> blockCenters = {};

  /// 매칭 블록의 좌측 중앙 글로벌 좌표. 요일 id → Offset.
  final Map<String, Offset> blockLefts = {};

  /// 매칭 블록의 우측 중앙 글로벌 좌표. 요일 id → Offset.
  ...
  /// (구) 매칭 블록의 중심 좌표 등록. 현재 사용처는 없지만 하위호환용으로 유지.
  void updateBlockCenter(String dayId, Offset? center) {
    if (center == null) {
      if (blockCenters.remove(dayId) != null) {
        notifyListeners();
      }
    } else {
  ...
  /// 매칭 블록의 좌측/우측 중앙 좌표를 등록.
  /// 두 값 모두 null이면 해당 요일 좌표를 완전히 제거.
